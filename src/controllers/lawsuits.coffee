...
module.exports =
  ...
  update: (req, res) ->
    ...
    Lawsuit.findOneAndUpdate conditions, data, (error, lawsuit) ->
      if error         
        if error.lastErrorObject?.code is 11001 then return res.send "ERROR: There already is a lawsuit with reference sign #{data.repository} #{data.number} / #{data.year}"
        else throw error
      {
        repository
        year
